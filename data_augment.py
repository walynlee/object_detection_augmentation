...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_path', type=str, default='big_img')
    parser.add_argument('--source_txt_path', type=str, default='big_label')
    parser.add_argument('--save_img_path', type=str, default='aug_img')
    parser.add_argument('--save_txt_path', type=str, default='aug_txt')
    args = parser.parse_args()
    source_img_path = args.source_img_path  # 图片原始位置
    source_txt_path = args.source_txt_path  # xml的原始位置
 
    save_img_path = args.save_img_path  # 图片增强结果保存文件
    save_txt_path = args.save_txt_path  # xml增强结果保存文件
    ...
    for parent, _, files in os.walk(source_img_path):
        ...
        for file in files:
            ...
            # xml_path = os.path.join(save_txt_path, file[:-4] + '.txt')
            # values = toolhelper.parse_xml(xml_path)  # 解析得到box信息，格式为[[x_min,y_min,x_max,y_max,name]]
            label_path = os.path.join(source_txt_path, file[:-4] + '.txt')
            values = []
            with open(label_path, 'r') as f:
                line = f.readline()
                while line:
                    # 读取每一行，并去除换行符，得到的是一个字符串
                    line = line.strip('\n')
                    # 读取每一行的列表化形式，得到的是一个列表
            ...
            for value in values:
                x_center, y_center, img_width, img_height = value[1], value[2], value[3], value[4]
                x_min =max(0, x_center - img_width/2 * img.shape[1])
                y_min =max(0,  y_center - img_height/2 * img.shape[0])
                x_max =min(img.shape[1], x_center + img_width/2 * img.shape[1])
                y_max = min(img.shape[0],y_center + img_height/2 * img.shape[0])
                coords = [v[:4] for v in value]  # 得到框
                labels = [v[-1] for v in value]  # 对象的标签
                ...
                while cnt < need_aug_num:  # 继续增强
                    ...
                    box = convert(height, width, auged_bboxes_int[0])
                    
                    img_name = '{}_{}{}'.format(_file_prefix, cnt + 1, _file_suffix)  # 图片保存的信息
                    
                    # 保存增强后的图像和标注文件
                    output_img_path = os.path.join(save_img_path, img_name)
                    cv2.imwrite(output_img_path, auged_img)

                    label_name = os.path.splitext(img_name)[0] + '.txt'
                    print(img_name)
                    output_label_path = os.path.join(save_txt_path, label_name)
                    print(label_data)

                    label_data = [0, box[0], box[1], box[2], box[3]]
                    label_data = [str(i) for i in label_data]
